...
export default function Box(props) {
  ...
  async function handle_submit() {
    ...
    fetch(url + "/document/", requestOptions)
        .then((res) => res.json())
  ...
  <button class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-l" onClick={props.click}>
  ❌
  </button>
  <button class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-r" onClick={handle_submit}>
  ✅
  </button>
</div>
        <MarkdownEditor
          ...
          height="550px"
          onChange={(value, viewUpdate) => setMarkdown({content:value})}
        />
      </div>
    </div>
  );
